...
public sealed class WindowsOnnxRuntimePlatform : IOnnxRuntimePlatform
{
    ...
    public WindowsOnnxRuntimePlatform(ILogger<BgiOnnxFactory> logger)
    {
        _logger = logger;
    ...
    public InferenceSession CreateInferenceSession(BgiOnnxModel model, bool ocr = false)
    {
        ...
        if (!EnableCache)
            return CreateSession(model.ModalPath, model, false, providerTypes);

        var cached = GetCached(model, providerTypes);
        return cached == null
            ? CreateSession(model.ModalPath, model, true, providerTypes)
            : CreateSession(cached, model, false, providerTypes);
    }
